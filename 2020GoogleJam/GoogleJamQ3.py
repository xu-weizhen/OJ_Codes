...
for t in range(T):
    ...
    while notFinish:
        ...
        for i in range(len(time_list)):
            if isinstance(time_list[i], list):
                notFinish = True
                if time_list[i][0] < first:
                    first = time_list[i][0]
                    index = i
        
        if not notFinish:
            break
        elif time_list[index][0] >= cameron:
            cameron = time_list[index][1]
            time_list[index] = 'C'
        elif time_list[index][0] >= jamie:
            jamie = time_list[index][1]
            time_list[index] = 'J'
        else:
            break
    ...
    if notFinish:
        ...
        print('Case #{}: {}'.format(t + 1, ''.join(time_list)))
